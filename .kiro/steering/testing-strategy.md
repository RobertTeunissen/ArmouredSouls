...
## Spec #53 capture and Spec #54 Finance Center coverage
...
- **Frontend and E2E capture regression coverage** retains admin contract tests for `CycleControlsPage`, `RepairLogPage`, `EconomyOverviewPage`, and `AuditLogPage`, plus authenticated admin and battle/result Playwright flows.

Finance Center is critical financial reporting, so its replacement coverage is mandatory and partitioned by behavior:

- **Unit/property coverage** proves preparation-spanning financial Cycle 1, active=`totalCycles + 1`, sequence-ordered reconciliation including per-cycle sequence restarts, both statement equations, sale polarity, stored facility itemisation, repair single contribution, battle-allocation conservation, direct-net identity, Revenue Growth/forecast boundaries, pagination invariance, and opaque source-reference properties.
- **PostgreSQL Integration_Tier coverage** proves scheduled/admin serialized cutover parity and races, blocked-writer next-cycle resolution, one consistent `asOf` read view, zero settlement rows and completed boundaries, versioned API period/auth/validation/ownership contracts, cache isolation, repair links, stable-aggregated battle allocations, deterministic pagination, payload safety, and query-growth budgets.
- **Frontend component coverage** proves version/provenance handling, URL-backed periods, preparation/provisional labels, current-inclusive History/Season to Date, sale/purchase headings, independent panel errors, browser-local time/DST, exactly five robot metrics, full-period/page separation, keyboard tabs/pagination, and 320–1024px responsive transformations.
- **Playwright coverage** proves `/income`, `/finances`, and `/cycle-summary` migration, exactly one player Finance Center navigation entry, separate admin Economy, overview-first lazy loading, Current Cycle/Season-to-Date active points, 44px controls, visible focus, pagination notice, and no page overflow at required viewports.
- **Performance fixture coverage** records maximum-roster fixture facts, p95, query count, response bytes and cache state; it fails on per-robot finance/battle/repair query growth or a breached target. Feature-specific thresholds live in the Finance Center reporting contract, not as a generic pass snapshot here.

Finance Center test retirement is replacement-gated. Adapt `finances.test.ts`, `financesRouteValidation.test.ts`, `coverageManifest.test.ts`, and route-specific financial service coverage. Retain formula/property and cycle-execution streaming coverage. Retire legacy report/analytics assertions only after their version-1 replacement and redirect coverage is green; unrelated financial-flow E2E mutations remain. The exact file-by-file disposition is maintained in `docs/implementation_notes/finance-center-reporting-contract.md`.
