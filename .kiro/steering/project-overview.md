...
13. **Booking Office / Event Subscription System** - Per-robot subscription model gating participation in all battle events (league_1v1, tournament_1v1, tag_team, koth, league_2v2, league_3v3) through a single extensible Event Registry, with facility-level-driven cap curve (3 base + 1 per level), free switching, and per-robot lock-on-queued-battle rule (Spec #35)
14. **Daily Cron Schedule** - 10-slot daily schedule with heavy-mode spacing (1v1 League 08:00, 2v2 League 09:00, 3v3 League 14:00, KotH 13:00), reserved slots for future tournament/melee modes, and midnight settlement. All events run daily; subscriptions gate participation. Canonical slot map in `docs/architecture/PRD_SERVICE_DIRECTORY.md` § Cron Schedule (Spec #36)
15. **Team Battle Mode (2v2 and 3v3 League)** - Simultaneous N-vs-N combat with persistent Teams, Team Coordination ally effects (focus fire, shield regen, formation defence), shared LP-primary matchmaking, N× reward multiplier, daily cadence (2v2 at 09:00, 3v3 at 14:00 UTC), subscription-gated participation via Booking Office using `hasSubscription()` (allows immediate team formation after subscribing), eligibility feedback with `ineligibilityReason`/`ineligibilityDetail` shown on Dashboard and Team Management page (Spec #37)
