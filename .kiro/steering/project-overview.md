...
22. **Universal_Search_System** - Authenticated player-facing, search-only discovery for exactly three categories through `GET /api/search`: robots matched only by `Robot.name` and targeting `/robots/:id`, stables matched only by trimmed non-empty `User.stableName` (including generated/test stables and excluding `username`) and targeting `/stables/:userId`, and guide articles matched only through the existing `Guide_Search_Index` with reference-only results targeting `/guide/:sectionSlug/:articleSlug`. A shared `Player_Shell` renders one `Navigation` and one Search_Palette around every post-onboarding player route while leaving onboarding, authentication/front-page routes and `/admin` outside the shell. The desktop navbar exposes a visible `Search` control with a `⌘ K`/`Ctrl K` hint, and the mobile fixed top header exposes a visible search icon/button; the shortcut is an accelerator, not the sole discovery mechanism, and the mobile control is not in bottom navigation or the `More` drawer. Recent_Search_History is browser-local, limited to five unique recent queries, and is never sent to the backend. Search analytics are isolated in the dedicated `Search_Analytics_Store` rather than player responses or `audit_logs`; eligible completed searches write server-owned active-season events, and Season_Rollover purges them without a cross-season archive. Reporting is admin-only through `GET /api/admin/search-analytics/report` and is not exposed in the player shell or palette. The MVP deliberately defers the broader backlog scope of players/usernames, weapons, navigation pages/actions, command actions, battle reports/history, fuzzy matching, anonymous search, server-persisted history, selection/category tracking and cross-season analytics (Spec #56)

## Financial Capture and Audit Boundary (Spec #53)

**`User.currency` is the authoritative Credits balance.** Every current-economy mutation uses `Credit_Mutation_Service` and creates one atomic pair: one `FinancialLedger` accounting/reporting row and one `AuditLog` `financial_transaction` operational/security row sharing `financialEventId`. The final taxonomy is exactly `battle_income`, `streaming_revenue`, `repair_cost`, `facility_upgrade`, `weapon_purchase`, `weapon_sale`, `weapon_refinement`, `robot_creation`, `attribute_upgrade`, `achievement_reward`, `passive_income`, and `operating_costs`; `Financial_Breakdown` stores the amount-affecting facts and rounding needed to explain each row later.

`Prestige_Service` records positive stable-level awards as `prestige_change` `AuditLog` rows with unique `sourceEventId`; prestige never becomes a financial amount. `repair_cost` is paired per robot with mandatory `repairType` `manual` or `automatic`, while subtype-bearing `robot_repair` rows remain the repair display/count/subtype source. A byed robot may receive a separate pre-battle automatic repair; that spend is never attributed to its bye reward. `Settlement_Service` writes exactly one `passive_income` pair and one `operating_costs` pair per applicable stable/cycle, including zero-valued components. Booking Office subscription changes remain free and nonfinancial. Rows without a `financialEventId` are legacy history: they are not reconstructed, paired, relabelled, or used to claim complete paired coverage. See [`FINANCIAL_LEDGER_AUDIT_GUIDE.md`](../../docs/guides/FINANCIAL_LEDGER_AUDIT_GUIDE.md).

## Finance Center Reporting Boundary (Spec #54)

Finance Center is the implemented single player financial destination at `/income`; `/finances` remains a compatibility redirect and `/cycle-summary` redirects to its History view, while `/admin/economy` stays separate. The version 1 report contract is read-only and active-season scoped. Architecture documentation establishes implemented behavior; release/deployment evidence and blocking verification results are recorded separately in `docs/implementation_notes/finance-center-reporting-contract.md`. An absent record is neither evidence that the application cutover is pending nor a claim that an unrun check passed.

Under the canonical cycle contract, `CycleMetadata.totalCycles` means completed financial cycles and active cycle is `totalCycles + 1`. Financial Cycle 1 starts at Season Rollover, spans both preparation days and the first competitive/match day, and closes at the first competitive settlement. Scheduled/admin closure and all writers share a serialized boundary: paired settlement components, `cycle_end_balance`, `cycle_complete`, and `CycleSnapshot` are written under one number before completed count advances.

Every report envelope uses one database-consistent `asOf` view. Signed movement comes only from complete ledger/financial-audit pairs ordered by `(cycleNumber, sequenceNumber)`; `cycle_end_balance` and snapshot balance are checks, not statement lines. `User.currency` confirms Current Cycle only. Positive `weapon_sale` is investment proceeds, not earned Credits. A repair’s ledger amount enters reconciliation once; its linked `robot_repair` row supplies positive display amount, count, and subtype. Robot battle/bye amounts use conserved owner-scoped `BattleParticipant.credits`, never synthetic division. Missing/legacy evidence produces a typed non-monetary limitation.

The authenticated version 1 resources are overview report, history, robot summaries, and paginated owned-robot events. Responses expose normalized period, one `asOf`, provenance/finality, reconciliation/limitations, and opaque player-safe source references; they never expose raw financial identities, audit payloads, or cross-user data. Cache keys include stable, active season, normalized period, resource and page/order. Queries are set-based and detail is paginated. See [`finance-center-reporting-contract.md`](../../docs/implementation_notes/finance-center-reporting-contract.md) and [`PRD_INCOME_DASHBOARD.md`](../../docs/prd_pages/PRD_INCOME_DASHBOARD.md).
