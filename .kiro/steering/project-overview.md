...
`User.currency` is the authoritative Credits balance. From the ACC `Cutover_Cycle` onward, every current-economy mutation uses `Credit_Mutation_Service` and creates one atomic pair: one `FinancialLedger` accounting/reporting row and one `AuditLog` `financial_transaction` operational/security row sharing `financialEventId`. The final taxonomy is exactly `battle_income`, `streaming_revenue`, `repair_cost`, `facility_upgrade`, `weapon_purchase`, `weapon_sale`, `weapon_refinement`, `robot_creation`, `attribute_upgrade`, `achievement_reward`, `passive_income`, and `operating_costs`; `Financial_Breakdown` stores the amount-affecting facts and rounding needed to explain each row later.

`Prestige_Service` records positive stable-level awards as `prestige_change` `AuditLog` rows with unique `sourceEventId`; prestige never becomes a financial amount. `repair_cost` is paired per robot with mandatory `repairType` `manual` or `automatic`, while subtype-bearing `robot_repair` rows remain the `Repair_Spend` `Canonical_Source`. A byed robot may receive a separate pre-battle automatic repair; that spend is never attributed to its `Bye_Event` reward. `Settlement_Service` writes exactly one `passive_income` pair and one `operating_costs` pair per applicable stable/cycle, including zero-valued components. Booking Office subscription changes remain free and nonfinancial. Pre-cutover rows are `Legacy_Record` history: they are not reconstructed, paired, relabelled, or used to claim complete coverage. The implementation contract, reconciliation procedure, admin compatibility, and deferred `Financial_Page_Follow_On` are documented in [`FINANCIAL_LEDGER_AUDIT_GUIDE.md`](../../docs/guides/FINANCIAL_LEDGER_AUDIT_GUIDE.md); no player-guide content or financial-page UI changes are part of this work.
