...
`Prestige_Service` records positive stable-level awards as `prestige_change` `AuditLog` rows with unique `sourceEventId`; prestige never becomes a financial amount. `repair_cost` is paired per robot with mandatory `repairType` `manual` or `automatic`, while subtype-bearing `robot_repair` rows remain the `Repair_Spend` `Canonical_Source`. A byed robot may receive a separate pre-battle automatic repair; that spend is never attributed to its `Bye_Event` reward. `Settlement_Service` writes exactly one `passive_income` pair and one `operating_costs` pair per applicable stable/cycle, including zero-valued components. Booking Office subscription changes remain free and nonfinancial. Rows without a `financialEventId` are legacy history: they are not reconstructed, paired, relabelled, or used to claim complete paired coverage. The implementation contract, reconciliation procedure, admin compatibility, and deferred `Financial_Page_Follow_On` are documented in [`FINANCIAL_LEDGER_AUDIT_GUIDE.md`](../../docs/guides/FINANCIAL_LEDGER_AUDIT_GUIDE.md); no player-guide content or financial-page UI changes are part of this work.
