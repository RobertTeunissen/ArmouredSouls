...
- **Required accounting is fail-closed.** Lock and re-read `User.currency` where the operation can race, allocate `sequenceNumber` only through `withAuditSequence`, validate the taxonomy and typed `Financial_Breakdown`, then write the balance and both records before commit. A ledger/audit/sequence failure rolls back the balance. `recordLedgerEntry.ts`, best-effort enrichment, and `financial_ledger_active` suppression are legacy/pre-cutover behavior and must not be used to hide a required post-cutover failure.
...
- **The evidence boundary is forward-only.** The contract becomes authoritative at the selected `Cutover_Cycle` in `ACC` after all writers and blocking checks pass. Earlier rows remain `Legacy_Record` history, are not paired or relabelled, and are not reconstructed by scripts or fallback payloads.
