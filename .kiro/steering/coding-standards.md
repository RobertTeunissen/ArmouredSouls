...
### Financial mutation and audit pairing (Spec #53)

- **`User.currency` is the authoritative balance field.** Use “Credits” in domain prose, but use the exact Prisma field `currency` in code, queries, examples, and documentation. Do not introduce a parallel `User.credits` field or treat a ledger balance as mutable state.
- **Every current-economy currency mutation uses `Credit_Mutation_Service`.** This includes battle and bye rewards, per-robot streaming revenue, achievement rewards, purchases, upgrades, sales, refinements, repairs, settlement components, and charged administrative maintenance. Account creation, account reset, season rollover, and explicit balance purge are `Opening_Balance_Boundary` operations only; they are not current-economy events.
- **The final `Transaction_Taxonomy` is closed:** `battle_income`, `streaming_revenue`, `repair_cost`, `facility_upgrade`, `weapon_purchase`, `weapon_sale`, `weapon_refinement`, `robot_creation`, `attribute_upgrade`, `achievement_reward`, `passive_income`, and `operating_costs`. New writers must not emit `subscription_cost`, `prestige_award`, or `settlement_adjustment`.
- **One credit mutation produces one pair, not two mutations.** The balance update on `User.currency`, one `FinancialLedger` accounting/reporting row, and one `AuditLog` row with `eventType` `financial_transaction` share one non-null `financialEventId` and commit in one database transaction. The `Financial_Audit_Record` proves the operational/security event; it never updates the balance a second time.
- **Required accounting is fail-closed from deployment.** Lock and re-read `User.currency` where the operation can race, allocate `sequenceNumber` only through `withAuditSequence`, validate the taxonomy and typed `Financial_Breakdown`, then write the balance and both records before commit. A ledger/audit/sequence failure rolls back the balance. Never reintroduce `recordLedgerEntry.ts`, best-effort enrichment, or `financial_ledger_active` suppression.
- **Every amount is explainable from stored facts.** `Financial_Breakdown` records formula/version, typed inputs, modifiers, discounts or bonuses, operation order, precision, rounding mode, source identity, and final amount. Reports must not reconstruct old amounts from current facilities, prestige, fame, repair quotes, or formula code.
- **Repairs stay separate and per robot.** `repair_cost` requires `repairType` exactly `manual` or `automatic` in both financial records. `Manual_Repair`, `Automatic_Repair`, and charged admin maintenance create one financial pair and one subtype-bearing `AuditLog` `robot_repair` record per repaired robot; batch manual quotes are rounded per robot before summing. Repair spend remains sourced from `robot_repair` (`creditsCharged`), never from `battle_complete`, `robots.repairQuoteCredits`, or a subtype-losing ledger total. Automatic repair for a byed robot is a separate pre-battle repair event, not part of the `Bye_Event` reward.
- **Prestige is not currency.** Positive awards use `Prestige_Service` and one `AuditLog` `prestige_change` record with a unique `sourceEventId`, resulting prestige, source, cycle, and typed award facts. They never create a `FinancialLedger` row or enter a credit `amount`/`balanceAfter`.
- **Settlement has two explicit components.** `Settlement_Service` writes one `passive_income` pair and one `operating_costs` pair for every applicable stable and cycle, including zero-valued components with unchanged `balanceAfter`. The scheduler, admin bulk cycle, and supported daily-finance trigger delegate to this service; they must not maintain independent balance updates.
- **Booking Office changes are free.** `Subscription_Change` writes only its existing subscription/domain audit records. It has no financial pair and no `subscription_cost` writer.
- **The evidence boundary is identity-based.** Deployed current-economy writes always carry a `financialEventId`; rows without one remain `Legacy_Record` history, are not paired or relabelled, and are not reconstructed by scripts or fallback payloads.
...
- **The three things that are not a repair spend source:**
  ...
  - **Not `financial_ledger`.** The ledger is an accounting mirror; repair spend remains sourced from `robot_repair`, whose subtype-bearing payload is the canonical repair evidence.
...
- **A bye simulates nothing, in any mode.** No `simulateBattle`, `simulateBattleMulti`, `simulateTeamBattle` or `simulateTagTeamBattle` on a bye path, and detection happens *before* the absent side is loaded or fabricated. It follows that a bye reward can never damage a robot, generate a bye-attributed repair bill, or draw; a separately scoped automatic repair may still charge a byed robot before its scheduled event.
