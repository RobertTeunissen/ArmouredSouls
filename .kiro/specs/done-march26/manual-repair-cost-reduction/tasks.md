...
- [x] 1. Apply 50% manual repair discount in backend endpoint
  - [x] 1.1 Modify `POST /api/robots/repair-all` in `app/backend/src/routes/robots.ts` to apply `MANUAL_REPAIR_DISCOUNT = 0.5`
  ...
  - [x] 1.2 Write property tests for manual discount formula
    ...
    - Create `app/backend/tests/manualRepairDiscount.property.test.ts`
  ...
  - [x] 1.3 Write property tests for manual cost bounds
    ...
    - Add to `app/backend/tests/manualRepairDiscount.property.test.ts`

  - [x] 1.4 Write unit tests for manual repair discount
    - Create `app/backend/tests/manualRepairDiscount.test.ts`
...
- [x] 2. Update audit logging with repairType field
  - [x] 2.1 Extend `EventLogger.logRobotRepair()` in `app/backend/src/services/eventLogger.ts`
  ...
  - [x] 2.2 Pass `repairType: 'manual'` from `POST /api/robots/repair-all` in `app/backend/src/routes/robots.ts`
  ...
  - [x] 2.3 Pass `repairType: 'automatic'` from `repairAllRobots()` in `app/backend/src/services/repairService.ts`
  ...
  - [x] 2.6 Write regression unit tests for automatic repair path
    - Add to `app/backend/tests/manualRepairDiscount.test.ts`
...
- [x] 4. Add admin audit log endpoint for repair filtering
  - [x] 4.1 Create `GET /api/admin/audit-log/repairs` endpoint in `app/backend/src/routes/admin.ts`
...
- [x] 5. Update frontend cost display on robots page
  - [x] 5.1 Update `calculateTotalRepairCost()` and button label in `app/frontend/src/pages/RobotsPage.tsx`
  ...
  - [x] 5.2 Update repair confirmation modal in `app/frontend/src/pages/RobotsPage.tsx`
...
- [x] 6. Build admin Repair Log tab
  - [x] 6.1 Add `RepairLogEvent` and `RepairLogResponse` interfaces to `app/frontend/src/components/admin/types.ts`
    - Define interfaces matching the backend response shape from task 4.1
    - _Requirements: 7.5_

  - [x] 6.2 Create `RepairLogTab` component in `app/frontend/src/components/admin/RepairLogTab.tsx`
  ...
  - [x] 6.3 Register `RepairLogTab` in `app/frontend/src/pages/AdminPage.tsx`
  ...
  - [x] 6.4 Export `RepairLogTab` from `app/frontend/src/components/admin/index.ts`
