...
- [x] 1. Backend environment configuration and startup validation
  - [x] 1.1 Create `app/backend/src/config/env.ts` with `loadEnvConfig()` function
  ...
  - [x] 1.2 Write property test for env config loading (Property 1)
    ...
    - Test file: `app/backend/tests/envConfig.property.test.ts`
  ...
  - [x] 1.4 Integrate `loadEnvConfig()` into `app/backend/src/index.ts`
  ...
  - [x] 1.5 Write property test for CORS origin parsing (Property 3)
    ...
    - Test file: `app/backend/tests/cors.property.test.ts`
    - **Validates: Requirements 3.4, 19.1, 19.2**

- [x] 2. Rate limiting middleware
  - [x] 2.1 Create `app/backend/src/middleware/rateLimiter.ts`
  ...
  - [x] 2.2 Apply rate limiters in `app/backend/src/index.ts`
  ...
  - [x] 2.3 Write property tests for rate limiting (Properties 6, 7)
    ...
    - Test file: `app/backend/tests/rateLimiter.property.test.ts`
    - **Validates: Requirements 20.1, 20.2, 20.3, 20.5**

- [x] 3. Health endpoint enhancement
  - [x] 3.1 Enhance `/api/health` in `app/backend/src/index.ts`
  ...
  - [x] 3.2 Write property test for health endpoint (Property 8)
    ...
    - Test file: `app/backend/tests/health.property.test.ts`
    - **Validates: Requirements 12.1, 12.3**

- [x] 4. Structured logging with winston
  - [x] 4.1 Create `app/backend/src/config/logger.ts`
  ...
  - [x] 4.2 Create request logging middleware `app/backend/src/middleware/requestLogger.ts`
  ...
  - [x] 4.3 Integrate logger and request logging into `app/backend/src/index.ts`
  ...
  - [x] 4.4 Write property tests for logging (Properties 9, 10, 11)
    ...
    - Test file: `app/backend/tests/logging.property.test.ts`
...
- [x] 6. Frontend API client and URL migration
  - [x] 6.1 Create `app/frontend/src/utils/apiClient.ts`
  ...
  - [x] 6.2 Write property tests for API client interceptors (Properties 4, 5)
    ...
    - Test file: `app/frontend/src/__tests__/apiClient.property.test.ts`
    - **Validates: Requirements 2.3, 2.4**

  - [x] 6.3 Migrate `app/frontend/src/utils/userApi.ts` to use `apiClient`
  ...
  - [x] 6.4 Migrate `app/frontend/src/utils/matchmakingApi.ts` to use `apiClient`
    - Remove local `API_BASE_URL` constant and `getAuthHeaders()` helper
    - Replace all API calls with `apiClient` using relative paths
    - _Requirements: 1.4, 2.2_

  - [x] 6.5 Migrate `app/frontend/src/utils/financialApi.ts` to use `apiClient`
    - Remove local `API_BASE_URL` constant and `getAuthHeaders()` helper
    - Replace all API calls with `apiClient` using relative paths
    - _Requirements: 1.4, 2.2_

  - [x] 6.6 Migrate `app/frontend/src/contexts/AuthContext.tsx` to use `apiClient`
  ...
  - [x] 6.7 Create `app/frontend/.env.example`
  ...
  - [x] 6.8 Verify zero hardcoded `localhost:3001` references remain in frontend source
    - Search all files under `app/frontend/src/` for `localhost:3001`
...
- [x] 8. Seed data strategy
  - [x] 8.1 Modify `app/backend/prisma/seed.ts` for multi-environment support
  ...
  - [x] 8.2 Write property tests for seed idempotence and filtering (Properties 12, 13)
    ...
    - Test file: `app/backend/tests/seed.property.test.ts`
    - **Validates: Requirements 15.1, 15.2, 15.3**

- [x] 9. Cycle Scheduler implementation
  - [x] 9.1 Create `app/backend/src/services/cycleScheduler.ts`
  ...
  - [x] 9.8 Integrate scheduler into `app/backend/src/index.ts`
  ...
  - [x] 9.9 Write property tests for scheduler (Properties 14–21)
    ...
    - Test file: `app/backend/tests/scheduler.property.test.ts`
...
- [x] 11. Infrastructure configuration files
  ...
  - [x] 11.5 Write property test for backup retention (Property 22)
    ...
    - Test file: `app/backend/tests/backup.property.test.ts`
    - **Validates: Requirements 8.2**

- [x] 12. Multi-environment configuration files
  - [x] 12.1 Create backend `.env.production.example` at `app/backend/.env.production.example`
  ...
  - [x] 12.2 Create environment-specific `.env` example files
    - `app/backend/.env.example` (DEV defaults)
    - `app/backend/.env.acc.example` (ACC values)
    - `app/backend/.env.prd.example` (PRD values)
    - `app/frontend/.env.test.example` (Playwright config)
...
- [x] 14. Test adaptation for multi-environment
  - [x] 14.1 Update `app/frontend/playwright.config.ts`
...
- [x] 16. Documentation
  ...
  - [x] 16.5 Update `app/README.md` with production deployment section
  ...
  - [x] 16.7 Document project structure decision and risk register
    - Record decision to retain `app/` structure with rationale
