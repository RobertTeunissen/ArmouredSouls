...
- [x] 1. Rename prototype directory and remove modules
  - [x] 1.1 Rename `prototype/` to `app/` using `git mv`
  ...
  - [x] 1.2 Remove the `modules/` directory
    - Run `git rm -rf modules/` from the repository root
    - Verify `modules/` no longer exists in the working tree
    - _Requirements: 2.1_

- [x] 2. Update CI/CD pipeline files
  - [x] 2.1 Update `.github/workflows/ci.yml`
  ...
  - [x] 2.2 Update `.github/workflows/deploy.yml`
...
- [x] 3. Update Kiro steering files
  - [x] 3.1 Update `.kiro/steering/project-overview.md`
  ...
  - [x] 3.2 Update `.kiro/steering/common-tasks.md`
    - Replace all `prototype/` path references with `app/`
    - _Requirements: 4.2_

  - [x] 3.3 Update `.kiro/steering/guide-content-maintenance.md`
    - Update the `fileMatchPattern` value from `prototype/backend` to `app/backend`
    - _Requirements: 4.3_

  - [x] 3.4 Update `.kiro/steering/environments-and-deployment.md`
    - Replace all `prototype/` path references with `app/` in setup commands and directory structure diagrams
    - _Requirements: 4.4_

  - [x] 3.5 Update remaining steering files with `prototype/` references
...
- [x] 4. Checkpoint - Verify steering and CI/CD updates
...
- [x] 5. Update root documentation
  - [x] 5.1 Update root `README.md`
  ...
  - [x] 5.3 Update `app/README.md` (formerly `prototype/README.md`)
...
- [x] 6. Update docs directory files
  - [x] 6.1 Update `docs/guides/MODULE_STRUCTURE.md`
    - Replace all `prototype/` path references with `app/`
  ...
  - [x] 6.2 Update remaining docs files with `prototype/` or `modules/` references
...
- [x] 7. Update GitHub Copilot instructions
  - [x] 7.1 Update `.github/copilot-instructions.md`
    - Replace all `prototype/` path references with `app/`
...
- [x] 8. Update in-progress spec references
  - [x] 8.1 Update spec 3 files under `.kiro/specs/to-do/3-backend-service-consolidation/`
    - Replace all `prototype/` path references with `app/` in `design.md`, `requirements.md`, and `tasks.md`
    - _Requirements: 7.1_

  - [x] 8.2 Update spec 5 files under `.kiro/specs/to-do/5-modular-architecture-migration/`
    - Replace all `prototype/` path references with `app/` in `design.md`, `requirements.md`, and `tasks.md`
    - Replace or remove `modules/` path references as appropriate
    - _Requirements: 7.1, 7.2_

  - [x] 8.3 Update spec 8 files under `.kiro/specs/to-do/8-battle-replay-admin/`
  ...
  - [x] 8.4 Update spec 9 files under `.kiro/specs/to-do/9-web-push-notifications/`
    - Replace all `prototype/backend` and `prototype/frontend` path references with `app/backend` and `app/frontend` in `design.md`
    - _Requirements: 7.1_

  - [x] 8.5 Update spec 4 files under `.kiro/specs/to-do/4-frontend-state-management/` if they contain `prototype/` references
    - Run `grep -rn "prototype/" .kiro/specs/to-do/4-frontend-state-management/` and update any matches
    - _Requirements: 7.1_

  - [x] 8.6 Update completed spec files under `.kiro/specs/done-april26/15-route-handler-extraction/`, `16-zod-validation-gaps/`, `17-type-safety-any-elimination/`, `18-frontend-component-splitting/`, `19-frontend-testing-foundation/`
  ...
  - [x] 8.7 Update spec 20 files under `.kiro/specs/to-do/20-robot-image-upload/` if they contain `prototype/` references
    - Run `grep -rn "prototype/" .kiro/specs/to-do/20-robot-image-upload/` and update any matches
    - _Requirements: 7.1_

  - [x] 8.8 Update GitNexus steering files under `.kiro/steering/gitnexus-*.md`
    - Replace all `prototype/backend` and `prototype/frontend` path references with `app/backend` and `app/frontend`
    - _Requirements: 4.5_

  - [x] 8.9 Update `AGENTS.md` and `CLAUDE.md` in the repository root
    - Replace all `prototype/` path references with `app/`
    - _Requirements: 5.4_

  - [x] 8.10 Update `.claude/skills/gitnexus/` skill files
    - Replace all `prototype/` path references with `app/`
    - _Requirements: 5.4_

- [x] 9. Update shell scripts and infrastructure files
  - [x] 9.1 Update `app/backend/scripts/fix-test-patterns.sh`
  ...
  - [x] 9.2 Verify infrastructure files need no content changes
...
- [x] 10. Checkpoint - Full reference verification
...
- [x] 11. Build and test verification
  - [x] 11.1 Verify backend build and tests
  ...
  - [x] 11.2 Verify frontend build and tests
...
- [x] 12. Final checkpoint - Verification criteria
