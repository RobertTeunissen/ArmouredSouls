...
/**
 * Manual test script to verify matchmaking system updates the database
 * 
 * Usage: npm run test:matchmaking
 * or: tsx scripts/testMatchmaking.ts
 */

import { PrismaClient } from '@prisma/client';
import { runMatchmaking } from '../src/services/matchmakingService';
import { getLeagueInstanceStats } from '../src/services/leagueInstanceService';
...
async function main() {
  ...
  for (const robot of robots.slice(0, 5)) {
    const hpPercent = (robot.currentHP / robot.maxHP * 100).toFixed(0);
    const hasWeapon = robot.mainWeaponId !== null;
    const isReady = (robot.currentHP / robot.maxHP >= 0.75) && hasWeapon;
    
    if (isReady) readyCount++;
    else notReadyCount++;

    console.log(`  ${robot.name}:`);
    console.log(`    HP: ${robot.currentHP}/${robot.maxHP} (${hpPercent}%)`);
    console.log(`    Weapon: ${hasWeapon ? '✓ Equipped' : '✗ Not equipped'}`);
  ...
  console.log(`Summary: ${totalReady}/${totalRobots} robots are battle-ready\n`);
  ...
  if (byeMatches.length > 0) {
    console.log('  Players getting bye-matches:');
    byeMatches.forEach(match => {
      const playerRobot = match.robot1.name !== 'Bye Robot' ? match.robot1.name : match.robot2.name;
      console.log(`    - ${playerRobot}`);
    });
  }
  console.log();

  // 7. League instance statistics
  console.log('🏆 LEAGUE INSTANCE STATISTICS');
  console.log('─────────────────────────────────────\n');

  const stats = await getLeagueInstanceStats('bronze');
  console.log(`Bronze League:`);
  ...
  console.log(`  Needs rebalancing: ${stats.needsRebalancing ? 'YES' : 'NO'}\n`);

  stats.instances.forEach(instance => {
    console.log(`  ${instance.leagueId}:`);
    console.log(`    Robots: ${instance.currentRobots}/${instance.maxRobots}`);
    console.log(`    Status: ${instance.isFull ? 'FULL' : 'Available'}`);
  });
  console.log();
  ...
  console.log(`✓ Battle-ready robots: ${totalReady}`);
