...
async function getHighestEloRobots() {
  try {
    ...
    if (robots.length > 0) {
      console.log(`Highest ELO: ${robots[0].elo} (${robots[0].name})`);
      console.log(`Lowest ELO in this list: ${robots[robots.length - 1].elo} (${robots[robots.length - 1].name})`);
    }
    ...
    robots.forEach(robot => {
      ...
      } else if (robot.name.includes('Velocity')) {
        typeCounts['Velocity (Speed)'] = (typeCounts['Velocity (Speed)'] || 0) + 1;
      } else if (robot.name.includes('Prestige Hunter')) {
        typeCounts['Prestige Hunter'] = (typeCounts['Prestige Hunter'] || 0) + 1;
      } else if (robot.name.includes('Speed Demon')) {
        typeCounts['Speed Demon'] = (typeCounts['Speed Demon'] || 0) + 1;
      } else {
