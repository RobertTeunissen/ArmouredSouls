...
describe('Preservation — Existing Behavior Unchanged', () => {
    ...
    test('env.ts scheduler defaults are preserved', () => {
        const envTs = readRootFile('prototype/backend/src/config/env.ts');
    ...
    test('.env.acc.example exists and is available', () => {
        const exists = existsInRoot('prototype/backend/.env.acc.example');
        expect(exists).toBe(true);
        // Verify it has content (not empty)
        const content = readRootFile('prototype/backend/.env.acc.example');
    ...
    test('.env.production.example exists as canonical production template', () => {
        const exists = existsInRoot('prototype/backend/.env.production.example');
        expect(exists).toBe(true);
        // Verify it has content (not empty)
        const content = readRootFile('prototype/backend/.env.production.example');
        expect(content.trim().length).toBeGreaterThan(0);
    });
    /**
    ...
    test('first /eligible-robots handler has correct comment and response shape', () => {
        const routeFile = readRootFile('prototype/backend/src/routes/adminTournaments.ts');
