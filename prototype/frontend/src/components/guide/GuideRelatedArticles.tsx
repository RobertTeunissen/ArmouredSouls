      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {relatedArticles.slice(0, 5).map((article) => (
          <Link
            ...
            className="flex items-center gap-3 p-3 rounded-lg border border-white/10 hover:border-primary/50 hover:bg-primary/5 transition-all"
          >
            <span className="text-primary">→</span>
            <div className="min-w-0">
              <div className="text-sm font-medium text-white truncate">{article.title}</div>
              <div className="text-xs text-tertiary">{article.sectionTitle}</div>
            </div>
          </Link>
        ))}
      </div>
