...
const WeaponSlotVisual = ({ label, content, isOccupied }: { 
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="text-xs text-gray-400 mt-1 text-center max-w-[80px]">
        {content}
      </div>
    </div>
  );
      ...
      {Object.entries(bonuses).map(([attr, value]) => {
        ...
        return (
          <div key={attr} className="flex justify-between text-sm">
            ...
            <span className={`font-medium ${bonus.isPositive ? 'text-green-400' : 'text-red-400'}`}>
              {bonus.text}
            </span>
          </div>
        );
...
const LoadoutDiagram = ({ loadoutType, showDetails = true, compact = false }: LoadoutDiagramProps) => {
      ...
      <div className="bg-surface rounded-lg border border-gray-700 p-4">
        ...
        {bonuses && Object.keys(bonuses).length > 0 && (
          <div className="border-t border-gray-700 pt-3">
            <h5 className="text-xs font-semibold text-gray-400 mb-2">Combat Bonuses</h5>
  ...
  return (
    <div className="bg-surface rounded-lg border border-gray-700 p-6">
      ...
      <div className="mb-6">
        <h4 className="text-sm font-semibold text-gray-300 mb-3">Weapon Slot Configuration</h4>
        <div className="flex justify-center gap-8 p-4 bg-gray-800 bg-opacity-50 rounded-lg">
          <WeaponSlotVisual 
            label="Main Hand" 
            content={config.mainSlot}
            isOccupied={mainOccupied}
        ...
        <p className="text-xs text-gray-400 mt-2 text-center">{config.description}</p>
      </div>
