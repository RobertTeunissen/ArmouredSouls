...
v1.12 (Sep 1, 2026): Spec #50 — generated tournament byes remain expected Upcoming Matches until current-round processing; resolved byes appear once in Recent Battles; shared all-mode Bye_Card terminology and durable markers documented.
...
- **Acceptance Criteria:**
  ...
  - **Bye matches**: Bracket generation creates a current-round Upcoming Match for the real robot/team with an expected credit display; it does not advance, award credits, or create Recent Battles history at generation time
    - When the current round is processed, the existing no-simulation bye path creates the Bye_Record and resolved Battle exactly once
    - Credits are awarded at processing: the same amount a loss pays for that round (Spec #49)
    - No prestige, fame, or streaming income
    - The participant advances only after the current round is terminal, and the next-round Match then becomes Upcoming
    - League, team, tag-team, KotH, and Grand Melee byes use the same no-simulation treatment; no mode fights a placeholder for display or reward purposes
  - All rewards are deposited immediately after the corresponding Battle or Bye_Record is resolved
...
1. Tournament Created → status: "pending"
   ...
   - First-round bye rows contain the real robot/team subject
   - Do not resolve, pay, or create Battle history for generated byes

2. Start Tournament → status: "active", currentRound: 1
   - Create current-round `scheduled_tournament_matches` rows
   - A generated bye remains an Upcoming Match with an expected reward

3. Execute the current round
   - Ordinary scheduled tournament Matches use the normal tournament Battle path
   - A current-round bye invokes the existing no-simulation bye resolution exactly once
   - The bye award, Bye_Record, and resolved Battle history are created at processing time

4. Advance after every current-round entry is terminal → currentRound: next round
   - Populate the next-round TournamentMatches with processed winners
   - Do not recursively resolve a newly generated bye
   - Create the next-round `scheduled_tournament_matches` rows
   - The participant's next-round Match is not Upcoming until this advancement occurs
   - Repeat until...

5. Final Round Completes → status: "completed"
   - Last Match determines the tournament winner
...
- The daily cycle triggers the active tournament's current round directly.
- Current-round entries, including generated byes, are represented by `scheduled_tournament_matches` for player-facing Upcoming Matches and lifecycle state.
- Ordinary entries execute through the normal tournament Battle path; a bye entry uses the existing no-simulation resolution path.
- All entries in a round reach a terminal state before winners advance and the next-round Match rows are exposed.
...
**Tournament Byes (no combat - tournament only):**
- Highest-seeded robots or teams in non-power-of-two brackets
- Created as current-round `isByeMatch: true` rows during bracket generation
- Remain Upcoming Matches with an expected credit reward until that round is processed
- Do not create credits, a resolved Battle, or Recent Battles history at generation time

**League Byes (scheduled walkovers):**
- A subscribed real robot or team has no real opponent in its event
- The existing scheduling and bye-resolution path records the resolved event
- The shared Bye_Card displays the real subject and no opponent
- Handled through the mode's existing scheduler/orchestrator path, not a tournament-only UI branch

**When Byes Occur:**
- Participant count not a power of two in a tournament (e.g., 350 participants → 512 bracket → 162 byes)
- Highest-seeded robots receive tournament byes based on ELO ranking
- A league, team, tag-team, KotH, or Grand Melee event can also produce a bye when its mode-specific field cannot supply a real opponent

**Bye Match Processing (updated by Specs #49 and #50):**
- **No combat occurs** — the bye is resolved without simulation
- **The current round owns resolution** — bracket generation only exposes an expected Upcoming Match
- **A Battle record is created at processing** — the resolved Bye_Record includes the participant, summary, audit, and scheduling links needed for history
- **Credits are awarded at processing** — the existing mode-specific bye reward path supplies the amount; no frontend formula is used
- **No prestige, fame, or streaming income** — a bye is not a fought Match
- **No damage** — nothing is simulated, so a bye cannot create a repair bill
- The real robot or team advances after the current round is terminal
- The next-round Match is exposed only after advancement

**Exactly-once and display behavior:**
- The current-round scheduling row and its resolved Battle link form the idempotency boundary. Retrying a round cannot pay or create a second bye history entry.
- Before processing, the shared `ByeMatchCard` shows BYE, the mode, the real robot/team/FFA subject, neutral no-opponent copy, and `byeRewardStatus: 'expected'`.
- After processing, Recent Battles shows the same resolved bye with `isByeMatch: true` and `byeRewardStatus: 'awarded'`.
- History uses durable `scheduled_matches_v2.is_bye_match` or `scheduled_tournament_matches.is_bye_match`, not the retained `battle_log`.
- The treatment is shared across `league_1v1`, `tournament_1v1`, `league_2v2`, `tournament_2v2`, `league_3v3`, `tournament_3v3`, `tag_team`, `koth`, and `grand_melee`.
