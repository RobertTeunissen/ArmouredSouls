...
description: "How single elimination tournaments work in Armoured Souls — bracket generation, ELO-based seeding, bye timing, and the path from first round to championship."
order: 1
...
Tournaments support three formats: **1v1** (individual robot participants), **2v2** (team participants), and **3v3** (team participants). All three use the same single-elimination bracket system described below, with team tournaments covered in detail in the [Team Tournaments](/guide/tournaments/team-tournaments) article.
...
- Each round, two robots or teams battle when the bracket supplies two real sides
- The winner advances to the next round
- The loser is eliminated from the tournament
- A generated bye advances its real robot or team without combat when that round is processed
- The last robot or team standing wins the championship
...
Tournaments don't always have a perfect number of participants (8, 16, 32, etc.). When the count isn't a power of two, the system handles the gap with **bye Matches** — the top seeds receive a one-sided Match and advance when that round is processed.

For example, in a 12-robot tournament:
- 4 robots receive first-round bye Matches (seeds #1 through #4)
- 8 robots play in the first round, producing 4 winners
- The second round has 8 robots: 4 bye recipients + 4 first-round winners

### Generated Byes and Round Timing

Bracket generation creates the bye record for the current round, but does not resolve it. Before that round is processed:

- the real robot or team appears in **Upcoming Matches**;
- the card is marked **BYE** and shows an expected credit reward;
- no credits are awarded and no Battle appears in **Recent Battles**; and
- no opponent placeholder is shown.

When the tournament processor runs that round, the bye is resolved without combat and the credit award is written once. The resolved bye then appears in **Recent Battles**, and only after round advancement does the participant's next-round Match appear in **Upcoming Matches**. The same lifecycle applies to 1v1, 2v2, and 3v3 tournament byes.

See [Bye Matches](/guide/tournaments/bye-matches) for the full details on assignment, timing, and rewards.
...
Each round is processed during the daily cycle, so a 16-robot tournament takes four cycles to complete. A generated bye remains an expected Upcoming Match until its own round is processed; it is not paid or added to Recent Battles during bracket creation. The deeper you go, the tougher the competition — and the bigger the [rewards](/guide/tournaments/rewards).

```callout-warning
Tournament Battles follow the same combat rules as league Battles — [stances](/guide/combat/stances), [weapon loadouts](/guide/weapons/loadout-types), [malfunctions](/guide/combat/malfunctions), and [yield thresholds](/guide/combat/yielding-and-repair-costs) all apply. A bye is the exception to combat: it is a no-simulation walkover, not a fight against a placeholder.
```
