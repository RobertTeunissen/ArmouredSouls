...
/* ------------------------------------------------------------------ */
/*  Grand Melee League History Section                                  */
/* ------------------------------------------------------------------ */

function GrandMeleeLeagueHistorySection({ robotId, robotName }: { robotId: number; robotName: string }) {
  const [standing, setStanding] = useState<KothStandingData | null>(null);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    api.get<{ standing: KothStandingData | null }>(`/api/robots/${robotId}/grand-melee-standing`)
      .then((res) => {
        if (!cancelled) setStanding(res.standing);
      })
      .catch(() => {
  ...
  if (loading) {
    return (
      <div className="bg-surface rounded-lg p-6">
        <h3 className="text-lg font-semibold text-white mb-4">Grand Melee League Standing</h3>
        <div className="flex items-center justify-center py-8 text-secondary">
          Loading Grand Melee standing...
        </div>
      </div>
    );
  }
  ...
  if (!standing) {
    return (
      <div className="bg-surface rounded-lg p-6">
        ...
        <div className="flex flex-col items-center justify-center py-12 text-secondary">
          <span className="text-3xl mb-3" aria-hidden="true">💀</span>
          <p className="text-center">No Grand Melee standing available.</p>
          <p className="text-center text-sm mt-1">
            Subscribe to Grand Melee to start competing in free-for-all elimination battles.
          </p>
        </div>
      </div>
    );
