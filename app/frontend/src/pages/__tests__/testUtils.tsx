...
export function createMockPaginatedResponse(
  ...
  return {
    ...
    zoneMeta: {
      ...
      promotionCandidates: Math.floor(data.length * 0.1),
      effectivePromotionCandidates: Math.floor(data.length * 0.1),
      promotionBlockReason: null,
      totalEntities: data.length,
      totalInstances: data.length > 0 ? 1 : 0,
      activeInstances: data.length >= 10 ? 1 : 0,
      instancesBelowMinimum: data.length > 0 && data.length < 10 ? 1 : 0,
      smallestInstancePopulation: data.length,
      isChampion: false,
      isBronze: true,
    },
  };
